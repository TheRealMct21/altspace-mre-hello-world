...
export default class HelloWorld {
	...
	private cigar_man(user: MRE.User) {
		if (this.attachedCigs.has(user.id)) {
			this.removeHats(user);
		} else {
			this.generate_cigar(user);
			this.generate_col(user);
		}
	}
	private generate_cigar(user: MRE.User) {

		let cigarartifact = "";
		switch (this.config["cigar"]) {
			case 0:
				cigarartifact = "artifact:1800364499115442471";
				break;

			case 1:
				cigarartifact = "artifact:1800364510129684783";
				break;
				
			case 2:
				cigarartifact = "artifact:1919953576222785637";
				break;
		...
		this.attachedCigs.set(user.id, MRE.Actor.CreateFromLibrary(this.context, {

			resourceId: cigarartifact,

			actor: {
				name: "Cigar",
				// appearance: {
				// 	meshId: this.assets.createBoxMesh('button', 3, 3, 9).id
				// },
				grabbable: true,
				transform: {
					local: {
						position: { x: -.02, y: 0.06, z: 0.13 },
						// position: { x: 1, y: 1, z: 0 },
						scale: { x: 0.01, y: 0.01, z: 0.01 },
						rotation: MRE.Quaternion.FromEulerAngles(
							270 * MRE.DegreesToRadians,
